...
export function PricingTiers({ currentTier }: { currentTier: TierKey }) {
  ...
  const handleChoose = (tier: TierKey) => {
    ...
    startTransition(async () => {
      const result = await startCheckout(tier, cadence);
      if (result?.error) {
        ...
        {(Object.values(TIERS)).map((tier) => {
          ...
          return (
            ...
            >
              ...
              >
                {isCurrent ? "Current plan" : pendingTier === tier.key ? "Redirecting..." : `Choose ${tier.label}`}
              </button>
            </div>
          );
        })}
